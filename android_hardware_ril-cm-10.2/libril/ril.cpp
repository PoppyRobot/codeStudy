...
typedef struct {
    int requestNumber;    // 表示当前命令的编号
    void (*dispatchFunction) (Parcel &p, struct RequestInfo *pRI);           // dispatchFunction的作用是，当前命令可以通过这个接口把数据发送到reference库；
    int(*responseFunction) (Parcel &p, void *response, size_t responselen);  // responseFunction的作用是：当Modem返回数据后reference侧可以用这个函数把数据进行打包，然后传递给Event侧。
} CommandInfo;
...
processCommandBuffer(void *buffer, size_t buflen) {
	...
	// 得到请求码和令牌
	// 最前面的是request号, 以及token域(request的递增序列号)。
	// request号是一个CommandInfo,它在ril_command.h中定义。
	...
	// 在processCommandBuffer中要做的就是通过当前的命令号，找到对应的发送函数（dispatchFunction）和打包函数（responseFunction）。
	// 这三个数据连同当前命令的令牌（当前命令的ID）构建一个在Event和reference侧通用的数据类型（RequestInfo）并把它发送给reference侧
...
/*    sLastDispatchedToken = token; */
	...
	// 这个RequestInfo会被挂入pending的request队列, 执行具体的dispatchFunction(), 进行详细解析
	// 调用reference  中的
...
// 发送命令给Modem是通过processCommandBuffer实现的
static void processCommandsCallback(int fd, short flags, void *param) {
	...
	// processCommandsCallback()通过record_stream_get_next阻塞读取s_fdCommand上发来的数据, 直到收到一完整的request。
	// 然后将其传递进processCommandBuffer()函数，processCommandBuffer()正式进入了命令的解析部分。
    for (;;) {
...
static void listenCallback (int fd, short flags, void *param) {
	...
	// 获取传入的socket描述符,也就是上层的java RIL传入的连接。
	...
	// 通过record_stream_new()建立起一个RecordStream, 
	// 将这个record_stream与s_fdCommand绑定, RecordStream实际上是一个用于存放数据的结构体，
	// 这个结构体提供了一些操作类来保证这个RecordStream所绑定的文件描述符被读取时里面的数据会被完整读取。
    p_rs = record_stream_new(s_fdCommand, MAX_COMMAND_BYTES);

	// 一旦s_fdCommand中有数据，它的回调函数processCommandsCallback()将会被调用
	// 再次把当前命令放入Eventloop
