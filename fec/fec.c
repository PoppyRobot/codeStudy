...
int main(int argc, char *argv[])
{
	...
	int fbits[300] = {0};
	unsigned char enc[300] = {0};
	unsigned char dec[94];
	int i = 0;
	
	void *fec = NULL;
	/* create fec */
	if((fec = create_viterbi27_port(300/2)) == NULL) {
		printf("error: create_viterbi27 failed\n");
		return -1;
	}
	/* set polynomial */
	...
	init_viterbi27_port(fec, 0);
	for (i = 0; i < 300; i++)
		enc[i] = (fbits[i] == 1) ? 0 : 255;
		
	update_viterbi27_blk_port(fec, enc, 300/2);
	chainback_viterbi27_port(fec, dec, 300/2, 0);
